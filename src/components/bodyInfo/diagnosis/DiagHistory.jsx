...
/* CSS */
import styles from "./DiagHistory.module.css";
import DiagChart from "../../bitComp/DiagChart";

function DiagHistory({ data }) {
  let hist = data.diagnosis_history;
  let month, year, heart, resp, temp, bloodPressure, systolic, diastolic;

  let months = hist.map((el) => {
    return (month = el.month);
  });
  let bloodPressures = hist.map((el) => {
    return (bloodPressure = el.blood_pressure);
  });
  let temps = hist.map((el) => {
    return (temp = el.temperature);
  });
  let resps = hist.map((el) => {
    return (resp = el.respiratory_rate);
  });
  let years = hist.map((el) => {
    return (year = el.year);
  });
  let hearts = hist.map((el) => {
    return (heart = el.heart_rate);
  });
  let systolics = bloodPressures.map((el) => {
    return (systolic = el.systolic.value);
  });
  let diastolics = bloodPressures.map((el) => {
    return (diastolic = el.diastolic.value);
  });
  let sys = bloodPressures.map((el) => {
    return (systolic = el.systolic.levels);
  });
  let dias = bloodPressures.map((el) => {
    return (diastolic = el.diastolic.levels);
  });
